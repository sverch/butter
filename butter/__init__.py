    ...
    def graph(self):
        ...
        def add_path(graph_string, from_node, to_node, protocol, port):
            if not from_node.name:
                ...
                from_string = ",".join(cidr_blocks)
            else:
                from_string = from_node.name
            graph_string += ("    \"%s\" -> \"%s\" [ label=\"(%s:%s)\" ];\n" % (from_string,
                                                                                to_node.name,
                                                                                protocol, port))
            return graph_string

        def add_node(graph_string, node_name):
            graph_string += "    \"%s\";\n" % node_name
            return graph_string
        ...
        for network_info in networks_info:
            ...
            graph_string = start_cluster(graph_string, network_info.name, network_info.name)
            ...
            if network_info.name not in net_to_service and network_info.name not in net_to_path:
                graph_string = add_node(graph_string, "Empty Network %s" % network_info.name)
                continue
            ...
            # networks.  Note we already have name collision issues with this graphviz output so
            # this doesn't make anything worse (i.e. if we have two different VPCs with the same
            # service names this breaks).
